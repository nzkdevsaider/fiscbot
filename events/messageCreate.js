...
module.exports = {
	...
	async execute(message) {
		...
		/**
		 * @type {Object}
		 * @description Biblioteca de funciones relacionadas con la base de datos.
		 */

		let data = {};

		data.getUser = async (id) => {
			if (typeof id !== "string")
				throw Error("El párametro ID tiene que ser un string.");
			return await client.findOrCreateUser({ id: id });
		};
		...
		try {
			command.execute(message, args, data, embed);
		} catch (error) {
			console.error(error);
			message.reply({
